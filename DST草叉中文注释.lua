local assets =	--材质 可以看到 本来是打算也做黄金的
{
...
local function onequip(inst, owner)	--拿起
...
local function normal()
    ...
    inst.components.finiteuses:SetConsumption(ACTIONS.TERRAFORM, .125)	--组件.有限使用:消耗方式【TERRAFORM,0.125】
    ------- 和铲子的一样 铲子不应该会有这个功能 但是却有了 

    inst:AddComponent("weapon")
    inst.components.weapon:SetDamage(TUNING.PITCHFORK_DAMAGE)

    inst:AddInherentAction(ACTIONS.TERRAFORM)	--规定动作【TERRAFORM】
	--这应该是规定动作的原因
	--不能使用的功能就不能不消耗 可能 本来铲子也有这个功能
