--翻译：黑夜里的黑猫
--这个猪人文件是我通过代码和游戏中猪人的行为相结合进行猜测和翻译的，未经过测试，如有差错，见谅
--由于本人有事，暂时无法继续研究，这个文件不知道会不会更新，见谅
local assets =
{
...
local function ShouldAcceptItem(inst, item)
	--如果物品的组件是可装备的，而且不是空的，（应该是给猪人装备上）并且返回真值
	if item.components.equippable ~= nil and item.components.equippable.equipslot == EQUIPSLOTS.HEAD then
		return true
	--否则，如果物品是可食用的，且不是空的，执行
...
--从玩家获取物品 参数（对象，给予者，物品）
local function OnGetItemFromPlayer(inst, giver, item)
	--I eat food
	--物品的组件的可食用属性是不是不为空，为空，结束，不为空执行下面
	if item.components.edible ~= nil then
		...
		if item.components.edible.foodtype == FOODTYPE.MEAT or item.components.edible.foodtype == FOODTYPE.HORRIBLE then
			...
			if inst.components.combat:TargetIs(giver) then
				...
				--否则如果战斗目标不是给予者且给予者的组件中的领导属性不为空，且inst对象不是守卫或者给予者不是怪物
			elseif giver.components.leader ~= nil and not (inst:HasTag("guard") or giver:HasTag("monster")) then
				--则给予者设置事件“成为朋友”
				giver:PushEvent("makefriend")
				--给予者领导组件中的领导属性添加跟随inst对象。
				giver.components.leader:AddFollower(inst)
				--inst对象跟随属性添加忠诚时间 参数（可食用物品的饥饿值*每阶段猪人忠诚度的饥饿值）
	...
	--I wear hats
	--如果物品是可装备的且装备的属性位置为头部
	if item.components.equippable ~= nil and item.components.equippable.equipslot == EQUIPSLOTS.HEAD then
		--inst对象的库存栏装备头部属性赋值给current
		local current = inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HEAD)
		--如果current不为空，就从库存摘下物品
		if current ~= nil then
...
--通过攻击决定权限 参数（对象（一般为猪人），攻击者（玩家，蜘蛛等））
local function OnAttackedByDecidRoot(inst, attacker)
	--获取inst对象当前的想xyz位置
	local x, y, z = inst.Transform:GetWorldPosition()
	--   =寻找相似的实体 战斗mod（分享目标距离） 猪人   怪兽猪 猪守卫
